...
describe('ForumService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ForumService,
        { provide: getRepositoryToken(ForumPost), useValue: mockForumRepository },
        { provide: getRepositoryToken(ForumComment), useValue: mockCommentRepository },
        { provide: getRepositoryToken(ForumLike), useValue: mockLikeRepository },
        { provide: getRepositoryToken(ForumSavedPost), useValue: mockSavedPostRepository },
      ],
    }).compile();
  ...
  describe('addComment', () => {
    it('should create and save a comment', async () => {
      ...
      mockCommentRepository.create.mockReturnValue({ userId: 1, postId: 1, ...data });
      mockCommentRepository.save.mockResolvedValue({ id: 1, userId: 1, postId: 1, ...data });
